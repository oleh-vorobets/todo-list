...
const signup = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
        if(password !== passwordConfirm) {
            return next(new AppError('Password and passoword confirmation are not match', 400));
        }
        const newUser = await User.create({ email, password });
        const token: string = createToken(newUser._id);
        const maxAge: number = +process.env.EXPIRE_TIME! || 3600;
...
const login = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if(!email || !password) {
            return next(new AppError(`'Email' and 'Password' fields can not be empty!`, 404));
        }
...
const forgotPassword = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const hashedToken: string = await bcrypt.hash(resetToken, process.env.SALT as string);
        ...
        const link: string = `${req.hostname}/${req.baseUrl}/reset-password?token=${resetToken}&id=${currentUser._id}`;
        const result: AppError | void = await sendEmail(email, 'Reset Password', `Hi dude!\n Your requested to reset
...
const resetPassword = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if(!passwordResetToken) {
            return next(new AppError("Invalid or expired password reset token", 404));
        }
        ...
        const link: string = `${req.hostname}/${req.baseUrl}/update-password/${user!._id}`;
        const result: AppError | void = await sendEmail(user!.email, 'Password Reset Successfully', `This is your temporary password ${password}.\n To change it go there ${link}`);
...
const updatePassword = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const {email, password, newPassword, newPasswordConf}: {email: string, password: string, newPassword: string, newPasswordConf: string} = req.body;
        const user = await User.findOne({ email });

        if(!user) {
            return next(new AppError('User with such email was not found', 404));
        }
