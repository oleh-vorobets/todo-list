...
export async function getTasks(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
        const sortQuery: string | undefined = req.query.sort as string;
...
export async function getTask(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    try {
        ...
        return next(new AppError(`Error retrieving task with id ${taskId}. Invalid id!`, 400));
    }
}
...
export async function updateTask(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    try {
        ...
        if(task && res.locals.user._id.equals(task.user)) {
            ...
            const updatedTask = await Task.findByIdAndUpdate(taskId, data, {new: true}).select('-__v -user');
            res.status(200).json({
                status: "success",
                data: {
                    data: updatedTask,
        ...
        next(new AppError(`Error retrieving task with id ${taskId}. Invalid id!`, 400));
    }
}
...
export async function deleteTask(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    try {
        ...
        if(task && res.locals.user._id.equals(task.user)) {
            const deletedTask = await Task.findByIdAndDelete(taskId).select('-__v -user');
            res.status(200).json({
                status: "success",
                data: {
                    data: task,
